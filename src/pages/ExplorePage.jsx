...
const TableLineChart = ({ changePct, coinId }) => {
	const { W, H } = generateSparklineData(changePct, coinId, 64, 32);
...
const StatChart = ({ changePct, statId }) => {
	const { W, H } = generateSparklineData(changePct, statId, 240, 60);
...
const ExplorePage = () => {
	...
	const fetchPrices = useCallback(async () => {
		try {
			const res = await fetch(`${API_URL}/api/crypto/all`);
	...
	const fetchTopGainers = useCallback(async () => {
		try {
			...
			setTopGainers(buildCoins(gainers));
		} catch (error) {
			console.warn('Failed to fetch top gainers:', error);
		}
	}, [buildCoins]);

	const fetchNewCoins = useCallback(async () => {
		try {
			const res = await fetch(`${API_URL}/api/crypto/new`);
			if (!res.ok) throw new Error(`Backend ${res.status}`);
			const newOnes = await res.json();
			setNewCoins(buildCoins(newOnes));
		} catch (error) {
			console.warn('Failed to fetch new coins:', error);
		}
	}, [buildCoins]);

	const buildCoins = useCallback((cryptos) => {
		const rate = getRate();
		return cryptos
			.map((crypto) => {
				const priceUsd = crypto.price;
				const price = priceUsd * rate;
				const change24h = crypto.change24h || 0;
				// Estimate market cap and volume since backend doesn't provide them
				const marketCap = priceUsd * 100000000; // rough estimate
				const totalVolume = priceUsd * 10000000; // rough estimate

				return {
					id: crypto.symbol,
					name: crypto.name,
					symbol: crypto.symbol,
					image: crypto.image || getCoinIcon(crypto.symbol),
					current_price: price,
					price_change_percentage_24h: change24h,
					market_cap: marketCap * rate,
					total_volume: totalVolume * rate,
				};
			})
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const cryptos = await fetchPrices();
				if (!cancelled) {
					setCoins(buildCoins(cryptos));
					setLoading(false);
				}
			} catch {
		...
		// Fetch top gainers and new coins
		fetchTopGainers();
		fetchNewCoins();
		...
		const id = setInterval(async () => {
			try {
				const cryptos = await fetchPrices();
				if (!cancelled) setCoins(buildCoins(cryptos));
				fetchTopGainers();
				fetchNewCoins();
			} catch { /* silent */ }
		}, 5000);
		return () => { cancelled = true; clearInterval(id); };
	}, [fetchPrices, buildCoins, fetchTopGainers, fetchNewCoins]);
