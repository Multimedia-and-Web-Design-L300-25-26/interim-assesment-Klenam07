...
const StepVerifyEmail = ({ email, onNext }) => {
	...
	const submitCode = async (codeArray) => {
		...
		try {
			const response = await axios.post(`${CONFIG.API_BASE_URL}/auth/verify-email`, {
				email: email.trim(),
	...
	const handleResend = async () => {
		...
		setError('');
		try {
			await axios.post(`${CONFIG.API_BASE_URL}/auth/resend-code`, { email: email.trim() });
			setTimer(30);
			setCode(['', '', '', '', '', '']);
		} catch (err) {
			...
			<div className="flex gap-3 mb-6">
				{code.map((d, i) => (
					<input key={i} ref={(el) => (refs.current[i] = el)} type="text" inputMode="numeric" maxLength={1}
						...
						onPaste={i === 0 ? handlePaste : undefined} disabled={isLoading}
						className={`w-12 h-14 text-center text-xl font-semibold rounded-xl border bg-[#1E2025] text-white outline-none transition-colors ${d ? 'border-[#0052FF]' : 'border-[#2C2F36]'} focus:border-[#0052FF] disabled:opacity-50`}
					/>
				))}
			</div>
...
const SignUp = () => {
	...
	switch (step) {
		...
		case 10: return <VerifyAddressStep onFile={file => { setAddressFile(file); setStep(11); }} />;
